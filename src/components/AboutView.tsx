  ...
  return (
    <article className="space-y-12 max-w-5xl mx-auto" id="about-view-root">
      ...
      <hgroup className="text-center py-6 border-b border-forest-100" id="about-heading-group">
        ...
        <p className="text-xs text-forest-400 mt-2 italic">Založeno pro spojení tradice, víry, odpovědnosti a odborné lesnické péče.</p>
      </hgroup>

      {/* Grid: Narrative on the left, core pillars on the right */}
      <div className="grid lg:grid-cols-12 gap-8 items-start" id="about-grid-content">
        
        {/* Main Narrative - customizable settings */}
          ...
          {[
            {
              ...
              desc: 'Transformujeme monokultury na druhově pestré, větru a kůrovci odolnější smíšené lesy plné jedlí, buků a dubů.',
            },
            {
              icon: Shield,
              title: 'Záruka trvalé udržitelnosti',
              desc: 'Certifikace PEFC zaručuje, že objem těžby nikdy nepřekročí přirozený přírůstek dřevní hmoty.',
            },
            {
              icon: Heart,
              title: 'Podpora komunity a diecéze',
              desc: 'Výtěžek hospodaření financuje opravu farních kostelů, chod církevního školství a charitativní pomoc potřebným v Jižních Čechách.',
            },
            {
              icon: Award,
              title: 'Profesionální lesnický tým',
              desc: 'Práci v terénu zajišťují zkušení revírníci s hlubokou znalostí místních poměrů a úctou k lesní půdě.',
            },
          ].map((pillar, index) => {
      ...
      {/* Decorative Quote */}
      <blockquote className="bg-forest-900 text-white rounded-xl p-8 text-center relative overflow-hidden border border-forest-950" id="about-quote-block">
        ...
        <q className="relative z-10 font-serif text-lg sm:text-xl md:text-2xl italic font-medium leading-relaxed block max-w-2xl mx-auto" id="about-quote-text">
          „Člověk je správcem stvoření, nikoliv jeho bezohledným majitelem. V lese pracujeme s vědomím, že co zasadíme dnes, sklidí naši vnuci.“
        </q>
        <cite className="relative z-10 block text-xs tracking-wider uppercase font-mono text-forest-300 mt-4 font-semibold" id="about-quote-author">
