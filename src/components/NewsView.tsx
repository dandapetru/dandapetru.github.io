...
export default function NewsView({ news, selectedNewsId, onSelectNews }: NewsViewProps) {
  ...
  // Pokud je zobrazena detailní stránka 
  if (currentArticle) {
    return (
      <article className="max-w-4xl mx-auto bg-white rounded-xl border border-forest-100 overflow-hidden shadow-sm" id="news-detail-container">
        ...
        <div className="p-6 sm:p-10 space-y-6">
          ...
          {/* Paticka*/}
          <div className="pt-6 border-t border-forest-100 flex flex-col items-center gap-3 sm:flex-row sm:justify-between sm:items-center" id="news-detail-footer">
            <button
              onClick={() => onSelectNews(null)}
              className="w-full sm:w-auto px-5 py-2.5 bg-forest-50 hover:bg-forest-100 border border-forest-200 hover:border-forest-300 rounded-lg text-[10px] sm:text-sm font-semibold text-forest-700 transition flex items-center justify-center gap-2 cursor-pointer"
              id="news-btn-back"
            >
              <ArrowLeft className="h-4 w-4" />
              Zpět na přehled aktualit
            </button>
            <span className="text-[11px] sm:text-xs text-forest-400 italic text-center sm:text-left">Jihočeské katolické lesy s.r.o.</span>
          </div>
        </div>
  ...
  return (
    <div className="space-y-8" id="news-main-container">
      ...
      {/* Nadpis */}
      <hgroup className="text-center py-6 border-b border-forest-100" id="news-heading-group">
        <span className="text-xs font-mono font-bold uppercase tracking-widest text-forest-500">
          Zprávy & Novinky z lesů
        </span>
        <h1 className="font-serif text-3xl sm:text-5xl text-forest-950 font-semibold mt-2 tracking-wide" id="news-title">
          Aktuality a zprávy z lesů
        </h1>
        <p className="text-xs text-forest-700 mt-2 italic max-w-2xl mx-auto [text-shadow:_0_1px_2px_rgba(0,0,0,0.15)]" id="news-description">
